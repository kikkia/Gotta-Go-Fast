...
# Defines array for inputs
# 0: Left Trigger amount
# 1: Right Trigger amount
# 2: Left stick X amount
# 3: Left stick X negative
def getDrivingState():
    for i in range(0, 4):
        if -0.12 < drivingState[i] < .12:
            drivingState[i] = 0
...
def sample_first_joystick():
    ...
    def on_axis(axis, value):
        ...
        if axis == "left_trigger":
            left_speed = value
            j.drivingState[0] = value
        elif axis == "right_trigger":
            right_speed = value
            j.drivingState[1] = value
        elif axis == "l_thumb_x":
            if value > 0:
                j.drivingState[2] = value
                j.drivingState[3] = 0
            else:
                j.drivingState[3] = value
                j.drivingState[2] = 0
